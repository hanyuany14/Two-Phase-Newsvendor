...
class Simulation():
    def __init__(self, cost: int, price: int, salvage_value: int, full_df: pd.DataFrame, demand_df:pd.DataFrame):
        self.cost = cost
        self.price = price
    ...
    def experiment(self, Qk: list[int]):
        ...
        # ====訓練階段====
        Qk_hat_df_train = self.qk_hat.make_Qk_hat_df_with_known_Qk(
            demand_df_train, T, Qk
        )
        training_profits, training_results, training_stimulation_results = (
            self.perform_fold_training(training_df, demand_df_train, Qk_hat_df_train, Q_star)
        )

        # ====測試階段====
        Qk_hat_df_test = self.qk_hat.make_Qk_hat_df_with_known_Qk(
            demand_df_test, T, Qk
        )
        ...
        return train_profit_df, test_profit_df, training_stimulation_result_df, testing_stimulation_result_df


        
    def perform_fold_training(self, training_df, demand_df_train, Qk_hat_df_train, Q_star) -> dict[str, float]:
        """This is for single fold training.

        Args:
            training_df (_type_): _description_
            demand_df_train (_type_): _description_
            Qk_hat_df_train (_type_): _description_
    ...
    def perform_fold_testing(
        ...
        if results_df_1 is not None:
            ...
            test_results_df_1, test_stimulation_results_df_1 = self.s1_model.cal_test_fixed_F_fixed_R(
        ...
        if results_df_2 is not None and len(results_df_2) > 0:
            ...
            test_results_df_2, test_stimulation_results_df_2 = self.s2_model.cal_test_flexible_F_fixed_R(
    ...
    def prepare_data(self, full_df, demand_df, train_size=0.5):
        """準備一次性的訓練資料與需求資料切分"""
        self.training_df, self.testing_df = self.split_data_by_ratio(full_df, train_size)
        self.demand_df_train, self.demand_df_test = self.split_data_by_ratio(demand_df, train_size)
