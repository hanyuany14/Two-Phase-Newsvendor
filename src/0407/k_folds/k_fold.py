...
def perform_single(data_size):
    ...
    def __fully_flexible_beta_with_lasso_15(
        ...
        with gp.Model("profit_maximization", env=env) as model:
            ...
            model.write("s15_model_debug.lp")
            model.write("s15_model.mps")
            try:
                model.optimize()
    ...
    def __fully_flexible_beta_with_second_training_16(
        ...
        with gp.Model("profit_maximization", env=env) as model:
            ...
            model.write("s16_model_debug.lp")
            model.write("s16_model.mps")
            try:
                model.optimize()
...
LASSO_BETAS = [1]
FOLDS = [5]
LASSO_BETA_SECOND_TRAINS = [0.9, 0.8]
# FOLDS = [3, 5, 10]
...
os.makedirs("results_0407", exist_ok=True)

summary = []
for chunk_size, lasso_beta, folds, lasso_beta_second_training in itertools.product(CHUNK_SIZES, LASSO_BETAS, FOLDS, LASSO_BETA_SECOND_TRAINS):
    CHUNK_SIZE = chunk_size
    data_size = CHUNK_SIZE * folds
    LASSO_BETA = lasso_beta
    LASSO_BETA_SECOND_TRAIN = lasso_beta_second_training

    print(f"Running: chunk={chunk_size}, lasso={lasso_beta}, folds={folds}, lasso_second_train={lasso_beta_second_training}")
    train_df, test_df = perform_single(data_size)

    # 存檔路徑
    tag = f"chunk{chunk_size}_lasso{lasso_beta}_fold{folds}_lst{lasso_beta_second_training}"
    train_df.to_csv(f"results_0407/train_{tag}.csv", index=False)
    test_df.to_csv(f"results_0407/test_{tag}.csv", index=False)
...
summary_df.to_csv("results_0407/summary.csv", index=False)
print("All experiments finished. Summary saved to results_0407/summary.csv")
print(summary_df)
